...
char * create_params(char *params_file){
	...
	fprintf(stderr,"\33[34m Parameters\n");
	fprintf(stderr,"\33[34m Input earth model: %s\n",(earth_model_file==NULL)?"(none), using water+salt simple model":earth_model_file);
	fprintf(stderr,"\33[34m Output folder: %s\n",folder_path);
	fprintf(stderr,"\33[34m Boundary: T=%d, E=%d (points)\n",BOUND_T,SPONGE_WIDTH);
	fprintf(stderr,"\33[34m Dimensions: n1=%d, n2=%d, n3=%d (points)\n",n1,n2,n3);
	fprintf(stderr,"\33[34m Number of frames nf=%d\n",nf);
	fprintf(stderr,"\33[34m Damping: bc=%f\n",bc_b);
	fprintf(stderr,"\33[34m Sampling: dx=%f (m)\n",d1);
	fprintf(stderr,"\33[34m Time scale dt=%f (s)\n",dt);
	fprintf(stderr,"\33[34m Time steps per frame: jt=%d\n",jt);
	fprintf(stderr,"\33[34m Number of time steps: nt=%d\n",nt);
	fprintf(stderr,"\33[34m Frequency: fpeak=%f (Hz)\n",fpeak);
	fprintf(stderr,"\33[34m Source: (x,y,z) = (%d,%d,%d)\n\33[30m",isx,isy,isz);
	fprintf(stderr,"\33[34m Rerun receiver: %s\n\33[30m",(rerun_receiver)?"True":"False");
	fprintf(stderr,"\33[34m Subsurface offsets: %d\n\33[30m",num_subsurface_offsets);
