	...
	uint32_t *expected)
{
	...
	for (int i = 0; i < size; i++) {
		...
		int countOneWrap = 0;
		if (countOneState >= ComplexCounter_countOneMax) {
			countOneState = 0;
			countOneWrap = 1;
		}

		if (countTwoState < ComplexCounter_countTwoMax && countOneWrap == 1)
			countTwoState++;

		expected[i] = input[i] + countTwoState;
		countOneState += ComplexCounter_countOneInc;
...
int main()
{
	...
	// Run the streams
	printf("Running DFE.\n");
	ComplexCounter(size, input, countOneOut, countTwoOut, result);
