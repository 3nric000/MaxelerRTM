...
void DFEStructCPU(int size, int32_t *inA, int32_t *inB, int32_t *outData)
{
	for (int i = 0; i < size; i++) {
...
int main()
{
	...
	max_file_t *maxfile = DFEStruct_init();

	int burstLengthInBytes = max_get_burst_size(maxfile, "cmd_tolmem");

	printf("Loading DFE memory.\n");
	DFEStruct_writeLMem(size, 0, inA);
	DFEStruct_writeLMem(size, size, inB);

	printf("Running DFE.\n");
	DFEStruct(size, burstLengthInBytes);

	int32_t *expected = malloc(sizeBytes);
	DFEStructCPU(size, inA, inB, expected);

	printf("Reading DFE memory.\n");
	int32_t *outData = malloc(sizeBytes);
	DFEStruct_readLMem(size, 2 * size, outData);
