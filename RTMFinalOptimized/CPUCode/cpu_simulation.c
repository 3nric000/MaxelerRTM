...
void setup_dfe() {
	...
	sizepxy = size * stencilSize * sizeof(float);
	int index;

	controllerSize = size * stencilSize;
	controllerSize = (burst - ((controllerSize) % burst)) + controllerSize;

	controller = malloc(controllerSize * sizeof(uint32_t));
	ppresult = malloc(sizeBytes);
	px = malloc(sizepxy);
	py = malloc(sizepxy);
	scale_stream = malloc(sizeBytes);

	for (index = 0; index < size * stencilSize; index++) {
		px[index] = 0;
		py[index] = 0;
		controller[index] = !(index % 11) ? 1 : 0;
	}

	for (index = 0; index < size; index++) {
		scale_stream[index] = 1;
	}

	maxfile = CpuMain_init();
	...
	/*act = max_actions_init(maxfile, "default");
	 ...
	 max_set_param_uint64t(act, "sizepxy", stencilSize * size);
	 max_set_param_uint64t(act, "controllerSize", controllerSize);
	...
	printf("loading toLMem\n");
	max_set_param_uint64t(act, "address", 0);
	max_set_param_uint64t(act, "nbytes", controllerSize * sizeof(uint32_t));
	max_queue_input(act, "cpu_to_lmem", controller, controllerSize
			* sizeof(uint32_t));
	max_run(engine, act);
...
void do_step_damping(float *__restrict p, float *__restrict pp,
	...
	for (i3 = ORDER; i3 < n3 - ORDER; i3++) { //Loop over slowest axis
		...
		for (i2 = ORDER; i2 < n2 - ORDER; i2++) { //Loop over middle axis
			...
			for (i1 = ORDER; i1 < n1 - ORDER; i1++) { //Loop over fast axis
				...
				scale_stream[(i1) + (i2) * n1 + (i3) * n12] = scale;
	...
	max_queue_input(act, "scale", scale_stream, size * sizeof(float));
	...
	for (i3 = ORDER; i3 < n3 - ORDER; i3++) { //Loop over slowest axis
		for (i2 = ORDER; i2 < n2 - ORDER; i2++) { //Loop over middle axis
			for (i1 = ORDER; i1 < n1 - ORDER; i1++) {
				...
				scale_stream[(i1) + (i2) * n1 + (i3) * n12] = 1;
