 ...
 * Summary:
 *        Shows the design after it has been fixed using maxdebug.
 */
...
void SimpleMV5CPU(int size, int32_t *dataIn, int32_t *dataOut)
{
	for (int i = 0; i < size; i++)
...
int main()
{
	...
	int32_t *dataIn[SimpleMV5_M] , *dataOut[SimpleMV5_M];
	...
	for (int i = 0; i < SimpleMV5_M; i++) {
		SimpleMV5CPU(size, dataIn[i], expected);
